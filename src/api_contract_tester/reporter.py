"""Generate test reports in various formats (console, JSON, HTML, Markdown)."""
...
class Reporter:
    ...
    def console_report(self, results: list[dict[str, Any]]) -> str:
        """Generate a console report."""
        total: int = len(results)
        passed: int = sum(1 for r in results if r["passed"])
        failed: int = total - passed
        total_time: int = sum(r.get("response_time_ms", 0) for r in results)
        ...
        for r in results:
            tc = r["test_case"]
            status_icon: str = "PASS" if r["passed"] else "FAIL"
            lines.append("")
            lines.append(f"  [{status_icon}] {tc.method} {tc.path}")
    ...
    def json_report(self, results: list[dict[str, Any]]) -> str:
    ...
    def markdown_report(self, results: list[dict[str, Any]]) -> str:
        ...
        passed: int = sum(1 for r in results if r["passed"])
        failed: int = total - passed
        pass_rate: int = round(passed / total * 100) if total > 0 else 0
        total_time: int = sum(r.get("response_time_ms", 0) for r in results)
        ...
        for r in results:
            ...
            http_code: str = str(r.get("status_code", "-"))
            rt: str = str(r.get("response_time_ms", "-")) + "ms"
            msgs: str = "<br>".join(r.get("messages", []))
    ...
    def html_report(self, results: list[dict[str, Any]]) -> str:
        ...
        for r in results:
            tc = r["test_case"]
            status_class: str = "pass" if r["passed"] else "fail"
            status_text: str = "PASS" if r["passed"] else "FAIL"
            msgs: str = "".join("<li>" + m + "</li>" for m in r.get("messages", []))
            rows += f"""
...
<div class="footer">API Contract Tester</div>
</div>
</body>
</html>"""
